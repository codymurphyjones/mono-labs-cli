...
export function runForeground(
  ...
  const newEnv: NodeJS.ProcessEnv = { ...filteredEnv, ...envObj }
  const newCmd = replaceTokens(cmd, newEnv) as string
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code: number | null, signal: NodeJS.Signals | null) => {
      ...
      for (const match of buffer.matchAll(TOKEN_RX)) {
        const groups = (match as any).groups as { field: string; value: string } | undefined
        if (groups?.field) {
          setData(groups.field, groups.value)
        }
      }
